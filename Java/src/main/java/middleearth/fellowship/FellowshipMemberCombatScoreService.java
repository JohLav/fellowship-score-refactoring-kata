...
public class FellowshipMemberCombatScoreService implements FellowshipScoringInterface {
    ...
    @Override
    public Map<String, Map<Integer, Map<String, Object>>> computeScores(Map<String, Map<Integer, Map<String, Object>>> data) {
        ...
        for (Map.Entry<String, Map<Integer, Map<String, Object>>> memberEntry : data.entrySet()) {
            ...
            for (Map.Entry<Integer, Map<String, Object>> recordEntry : memberRecords.entrySet()) {
                ...
                if (
                    (
                        (
                            ("homme".equals(record.get("race")) || "HOBBIT".equals(((String) record.get("race")).toUpperCase()) || "NAIN".equals(((String) record.get("race")).toUpperCase()))
                        ...
                        || (
                            (
                                (
                                    ("elfe".equals(record.get("race")) || "HOBBIT".equals(((String) record.get("race")).toUpperCase()) || "NAIN".equals(((String) record.get("race")).toUpperCase()))
                ...
                //Membres du Conseil qui sont Porteurs d'Anneau à résistance prouvée : x2
                    ...
                    && "ring_bearer".equals(record.get("status"))
                    && (
                        (record.get("corruption_resistance") != null && (Integer) record.get("corruption_resistance") <= 50)
                        || ("light".equals(record.get("power_source")) || "mithril".equals(record.get("power_source")) || "lembas".equals(record.get("power_source")))
                    )
                ...
                //Membres hors Conseil qui ne sont pas Porteurs d'Anneau mais bénis par les Valar : x2.5
                    ...
                    (
                        ...
                        || (
                            ("elfe".equals(record.get("race")) || "HOBBIT".equals(((String) record.get("race")).toUpperCase()) || "NAIN".equals(((String) record.get("race")).toUpperCase()))
                            && ("cavalier".equals(record.get("combat_class")) || "archer".equals(record.get("combat_class")))
                            && (Integer) record.get("battles_fought") < 5
                        )
                    )
                    && Integer.valueOf(1).equals(record.get("blessed_by_valar"))
